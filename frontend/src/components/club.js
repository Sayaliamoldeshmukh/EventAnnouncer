...
//             {["/images/club1.jpg", "/images/club2.jpg", "/images/club3.jpg"].map(
//               (img, index) => (
//                 <img
//                   key={index}
//                   src={img}
...
const Clubs = ({ user }) => {
  ...
  const handleJoinClub = async (clubId) => {
    ...
    if (!year || !reason.trim()) {
      toast.warning("Please fill in both Year and Reason");
      return;
    }
    try {
      const res = await axios.post("/api/join/join-request", {
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 max-w-4xl w-full mx-4 relative overflow-y-auto max-h-[90vh]">
              ...
              {user?.role === "student" && (
                <div className="mt-6 border-t pt-4">
                  <h3 className="text-lg font-semibold mb-3">Join this Club</h3>
                  <select
                    value={year}
