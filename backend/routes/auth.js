...
router.post('/send-otp', async (req, res) => {
  ...
  await transporter.sendMail({ to: email, subject: 'Verify Your Email - OTP Inside', html });
  res.json({ message: 'OTP sent to your email. Check inbox/spam.' });
});
...
router.post('/signup', async (req, res) => {
  ...
  const [otpCheck] = await db.query('SELECT * FROM email_otps WHERE email = ?', [email]);
  if (otpCheck.length > 0) return res.status(400).json({ message: 'Please verify email before signing up.' });
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    let club_id = null;
    if (user.role === 'club_admin' && user.club_name) {
      const [club] = await db.query('SELECT id FROM clubs WHERE name = ?', [user.club_name]);
      if (club.length > 0) club_id = club[0].id;
    }
